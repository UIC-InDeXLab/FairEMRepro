...
class FairEM:
    ...
    def is_fair(self, measure, aggregate, real_distr=False):
        if len(self.workloads) == 1:
            ...
            if aggregate not in ["distribution-subtraction", "distribution-division"]:
            ...
            for workload in self.workloads:
                workloads_fairness.append(
                    workload.fairness(workload.k_combs, measure, aggregate)
                )
            ...
            # specific for each measure
                ...
                for i in range(len(self.workloads)):
                    ...
                    for j in range(len(fairnesses)):
                        ...
                        subgroup_to_list_of_fairneses[subgroup_name].append(
                            fairnesses[j]
                        )
                ...
                for subgroup in subgroup_to_list_of_fairneses:
                    if (
                        len(subgroup_to_list_of_fairneses[subgroup]) >= 30
                    ):  # limit for a valid z-test
                        p_value = ztest(
                            subgroup_to_list_of_fairneses[subgroup],
